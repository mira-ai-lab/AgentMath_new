...
本数据集是首个专为**大模型多智能体系统 (Multi-Agent Systems)** 设计的多模态数学推理基准。当前数学推理评估主要面临三大挑战：**模态单一化**、**评估结果化**以及**协作脆弱性**。

为了弥补这些空白，本数据集提供了 1,003 个高质量样本，不仅包含多模态（文本、图像、音频）输入，更引入了**分步推理里程碑 (Milestones)** 指标与**对抗性负样本 (Adversarial Negative Samples)**，旨在全方位评估多智能体协作中的信息对齐、逻辑推理及错误自愈能力。
...
本数据集根据模态和难度划分成不同难度的数据，评估多智能体系统的多模态处理能力和数学推理能力：
...
| **模态分布** | Text, Image, Image-Text，Audio | 4 类组合 |
...
```json
{
  ...
  "milestones_1":[ //此处包含参数与milestones_0相同，但解题思路不同，略
  ]
}
```
...
我们整体上从**编排结果**和**编排过程**两个维度，对多智能体系统进行系统且详细的评分：
...
2. 🤝 **任务分工质量得分**
考虑任务完整、依赖关系正确、步骤顺序正确，包含步骤冗余率和步骤缺失率两个具体指标：
- 🚫 步骤冗余率 = 多余步骤/总步骤
- ⚠️ 步骤缺失率 = 缺失步骤/总步骤

3. 🔧 **自我纠错**
考虑解决回溯、冲突等情况，包含以下三个指标：
- 💬 解决冲突沟通轮次
- 🔍 错误识别率 = 识别到的错误/总错误
- ✅ 错误改正率 = 改正的/识别到的错误
